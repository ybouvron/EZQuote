...
vector<vector<Point> > filterRectangles(Mat image, const vector<vector<Point> >& rectangles ) {
	...
	for(size_t i = 0; i < rectangles.size(); i++) {
    	...
    	if(rectangleArea < (imageSize * 0.7) && rectangleArea > (imageSize * 0.1)
       	&& centerPoints[i].x >= imageCenter.x - image.rows/5
       	&& centerPoints[i].x <= imageCenter.x + image.rows/5
       	&& centerPoints[i].y >= imageCenter.y - image.cols/5
       	&& centerPoints[i].y <= imageCenter.y + image.cols/5) {
...
static void chooseBestRectangle(vector<vector<Point> >& rectangles) {
	...
	cout << "average center point: " << avgPoint << endl;
            ...
            //NEW!
	float minX, maxX;
	minX = centerPoint[0].x;
	maxX = centerPoint[0].x;
	for(int i = 0; i < centerPoint.size(); i++) {
    if(centerPoint[i].x < minX){
   	 minX = centerPoint[i].x;
    }
    if(centerPoint[i].x > maxX){
   	 maxX = centerPoint[i].x;   	 
    }
	}
	bestX = maxX - minX;
	bestY = avgPoint.y;    
	...
	for(int i = 0; i < centerPoint.size(); i++){
    	if(contourArea(rectangles[i]) <= average*2) {
