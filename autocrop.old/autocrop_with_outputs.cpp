...
int main(int ac, char** av)
{
	...
	vector<vector<Point> > end_points; // End points of lines
	...
	//reduces image size by 1/3. Only for viewing purposes. Remove in final code
	...
	imageProcessed = imageScan(src, rectangles, alpha, beta);
	...
	approxPolyDP(corners, approx, arcLength(Mat(corners), true) * 0.02, true);
...
static void chooseBestRectangle(vector<vector<Point> >& rectangles) {
	...
	for(int i = 0; i < rectangles.size(); i++) {
		if(  areaDiff == 0.0 || (abs(contourArea(rectangles[i])) - average) < areaDiff  ) {
			areaDiff = abs(contourArea(rectangles[i]) - average);
			bestIndex = i;
		}		
	}
	temp = rectangles[bestIndex];
	rectangles.clear();
...
Mat imageScan(Mat src, vector<vector<Point> >& rectangles, double alpha, int beta) {
    ...
    double endAlpha = 1.1;    //Change this variable to adjust outer loop count. Max value: 3
    ...
    for (alpha = startAlpha; alpha < endAlpha; alpha += 0.1) {
        ...
        for(int thresh = 1; thresh < loopCount; thresh++) {
            ...
            findRectangles(tempImage, rectangles, thresh);    //Finds all rectangles
            rectangles = filterRectangles(tempImage, rectangles);    //Filters out large and small false detections
        }
